...
<div class="QA-main-content">
    ...
    <!-- Nav tabs -->
<ul class="nav nav-tabs question-search-head" role="tablist">
  <li role="presentation" class="active"><a href="#home" role="tab" data-toggle="tab">问题</a></li>
  <li role="presentation"><a href="#profile" role="tab" data-toggle="tab">用户</a></li>
</ul>

<!-- Tab panes -->
<div class="tab-content question-search-content">
  <div role="tabpanel" class="tab-pane active" id="home">
    <?php
 if($question_data==null){ echo '<section class="null-question">
          <h2 class="null-question-title">无相关问题</h2>
        </section>'; }else{ foreach ($question_data as $key => $value) { $showtime = time() - $value['time']; if ($showtime < 60) { $showtime = ($showtime)."秒前"; } elseif ($showtime / 3600 < 1) { $showtime = (floor($showtime / 60))."分钟前"; } elseif ($showtime / 3600 < 8) { $showtime = (floor($showtime / 3600))."小时前"; } else { $showtime = date("y-m-d h:i", $value['time']); } echo '
  ...
  <div role="tabpanel" class="tab-pane" id="profile">
    <?php
 if($user_data==null){ echo '<section class="null-question">
          <h2 class="null-question-title">无相关用户</h2>
        </section>'; }else{ foreach ($user_data as $key => $value) { $src="/Public".$value['user_image']; echo '<section class="user-list-section">
          <img src="'.$src.'" class="img-responsive img-thumbnail user-img" alt="">

          <a href="/index.php/home/user/index?id='; echo $value['id']; echo '" class="user"><strong>'.$value['user_name'].'</strong></a>
        </section>'; } } ?>
  </div>
</div>
